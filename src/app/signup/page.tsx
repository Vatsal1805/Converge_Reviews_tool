...
export default function SignUpPage() {
  ...
  async function handleEmailSignUp(e: React.FormEvent) {
    ...
    try {
      ...
      // Check role & check if admin or client
  ...
  return (
    <div className="min-h-screen bg-[#0B0F19] text-[#F8FAFC] flex flex-col justify-center items-center px-4 py-12 relative overflow-hidden font-sans">
      ...
      <div className="absolute -bottom-32 -right-32 w-96 h-96 bg-[#9C6B1F]/10 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="mb-8 text-center relative z-10">
        ...
        <p className="text-sm text-slate-400 max-w-xs mx-auto">
          Start capturing 5-star Google reviews automatically
        </p>
      </div>
      ...
      <div className="w-full max-w-md bg-[#131B2E]/90 backdrop-blur-xl border border-slate-800/80 rounded-2xl p-8 shadow-2xl relative z-10 shadow-black/50">
        <div className="mb-6">
          <h1 className="text-xl font-bold text-white mb-1">Create business account</h1>
          <p className="text-xs text-slate-400">Join business owners growing their Google ratings</p>
        </div>
        ...
        <form onSubmit={handleEmailSignUp} className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1.5">Full name / Business owner</label>
        ...
        <div className="mt-6 text-center text-xs text-slate-400">
          ...
          <Link href="/login" className="text-[#D97706] font-medium hover:underline">
            Sign in
          </Link>
        </div>
      </div>
      ...
      <div className="mt-8 flex items-center gap-2 text-xs text-slate-500 relative z-10">
        ...
        <span>Secured authentication — no credit card required to start</span>
      </div>
    </div>
  );
}
